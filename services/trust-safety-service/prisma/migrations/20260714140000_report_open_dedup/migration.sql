...
-- (anon-path rate-limit hardening is a separate follow-up). targetType values
-- are disjoint per owning service, so a backfilled merge of the three
-- per-service report tables can't collide on (targetType,targetId,reporterId).
-- Prisma's DSL can't express a partial/filtered unique index, so it lives here
-- as raw SQL only (see the note on Report in schema.prisma). Idempotent so a
-- re-run is a no-op.
