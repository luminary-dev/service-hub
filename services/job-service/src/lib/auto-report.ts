...
// report on the target so it surfaces in the admin moderation queue — the
// content stays visible (decision on #375: auto-report for triage, never
// hard-block a write). Targets here: JOB for job-post text, JOB_RESPONSE for
// a provider's response message. Dedupe: at most one OPEN SYSTEM report per
// target, refreshed with the latest match on re-edit. Best-effort — a
// moderation failure must never fail the user's write. review-service and
// provider-service keep the same-shaped helper for the content they own; only
// the target-type union differs.
import { db } from "../db";
import { log } from "./log";
