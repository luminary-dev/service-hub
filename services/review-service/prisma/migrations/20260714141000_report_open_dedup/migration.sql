-- #651: one OPEN report per (target, reporter). The report-create handler
...
-- (reporterId IS NULL) have no identity to key on and are intentionally excluded
-- — the gateway rate limiter is their backstop (anon-path rate-limit hardening
-- is a separate follow-up). Prisma's DSL can't express a partial/filtered unique
-- index, so it lives here as raw SQL only (see the note on Report in
-- schema.prisma). Idempotent so a re-run is a no-op.
