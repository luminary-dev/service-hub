...
// hard-block a write). Dedupe mirrors provider-service's threshold flagging
// (#232): at most one OPEN SYSTEM report per target, refreshed with the
// latest match on re-edit. Best-effort — a moderation failure must never fail
// the user's write. provider-service and job-service keep the same-shaped
