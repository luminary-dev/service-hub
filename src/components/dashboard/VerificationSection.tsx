...
export default function VerificationSection({
  ...
  if (status === "VERIFIED") {
    return (
      <div className="tech-corners mb-6 flex items-center gap-3 rounded-lg border border-brand-200 bg-brand-50 p-4">
  ...
  if (status === "PENDING") {
    return (
      <div className="tech-corners mb-6 flex items-center gap-3 rounded-lg border border-amber-200 bg-amber-50 p-4">
  ...
  return (
    <div className="tech-corners mb-6 rounded-lg border border-ink-200 bg-surface p-5">
      ...
      <form onSubmit={submit} className="mt-4">
        <FormRow>
          <Field label={t.nicLabel} htmlFor="vs-nicLabel">
            <input
              id="vs-nicLabel"
              ref={nicRef}
              type="file"
          ...
          <Field label={t.businessLabel} htmlFor="vs-businessLabel">
            <input
              id="vs-businessLabel"
              ref={bizRef}
              type="file"
        ...
        <p className="mt-4 text-xs text-ink-500">{t.privacyHint}</p>
        {error && (
