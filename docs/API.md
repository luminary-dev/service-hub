...
| `GET /api/providers/:id` | public | Legacy detail: provider + services + photos, contact as `user`. Suspended → 404 unless caller is ADMIN. |
| `GET /api/providers/:id/full` | public | Full profile payload: services, first 50 photos (`photosTotal`), first page of reviews (`?reviewsTake`≤100, `?reviewsCursor`; `reviewsNextCursor` returned), `avgResponseMs`, `favorited`. Suspended → 404 unless ADMIN. |
