...
| `GET /internal/providers/cards?ids=` | Batched public **card DTO** hydration (≤500) for search-service's query plane (search RFC §4.1) — the same card shape browse builds (cover fallback, services, photos, the optional `latitude`/`longitude` pin). Rating fields come from the provider's denormalized `ratingAvg`/`ratingCount` columns (#748); search-service overlays its own aggregates on top regardless. Suspended providers excluded. Order not meaningful. |
...
| `PUT /internal/providers/:id/rating` | Denormalized rating write-back (#748): review-service PUTs `{ ratingAvg, ratingCount }` on every review create/update/delete/moderation so the public directory can filter/sort/count DB-side without a per-request rating fan-out. `ratingAvg` is clamped to 0..5 and tolerates `null` (a zero-review provider → stored 0); `ratingCount` is a non-negative int. `updateMany`, so an unknown/since-erased provider is an idempotent no-op `200 { ok: true }` (a moderation write-back racing an account erase must not 500). |
| `POST /internal/providers/rating/backfill` | Deploy-time / self-healing reconcile for the denormalized aggregates (#748): walks every provider in id-cursor pages, pulls authoritative summaries from review-service's `/internal/ratings`, writes `ratingAvg`/`ratingCount` → `{ ok: true, updated, skipped }`. A page whose rating batch failed to load is **skipped, not zeroed**, so a transient review-service outage never wipes real aggregates. Idempotent; run once after the column migration and any time drift is suspected. See [OPERATIONS.md](../OPERATIONS.md#provider-rating-denormalization-backfill-748). |
