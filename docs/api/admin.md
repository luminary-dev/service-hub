...
| Method + path | Auth | Summary |
|---|---|---|
| `GET /api/admin/providers` | SUPPORT+ | Moderation list: `q`/`category`/`city`/`status`/`suspended` filters, sort `newest`\|`mostReviews`, paginated (default 20, cap 100) → `{ providers, total, page, pageSize }`. Each row carries the per-row `quality` score (#229 — `qualityScore` + `rating` + `reviewCount` + `openReportCount`, same shape as the detail route). `mostReviews` ranks in memory over at most the 1000 newest matches (#372). |
