...
| `GET /api/auth/oauth/:provider/start` | public | Social login (#398); `:provider` ∈ `google`, `facebook`. Sets state (+ PKCE) cookies, 302 → provider consent. Optional `?next=` (same-origin relative). **Mobile (#398):** `?client=mobile&redirect=baaslk://auth` remembers the app deep link (only the app's own `baaslk://` scheme is accepted) so the callback hands back tokens instead of a cookie. Unknown/unconfigured provider → 302 `/login?error=oauth_unavailable`. |
| `GET /api/auth/oauth/:provider/callback` | public | Validates state (+ PKCE) + code, reads the provider identity (Google: id_token; Facebook: Graph API), then: existing linked account → sign in; matching verified email → link + sign in; new verified email → create a CUSTOMER (`emailVerified` set) + link; no email (some Facebook accounts) → create a CUSTOMER keyed on the provider id with a non-deliverable placeholder email (never auto-linked). Web: sets cookie, 302 → `/welcome` (new) or `next`/`/` (returning). **Mobile flow:** mints the Bearer access+refresh pair (same as `POST /api/auth/token`) and 302 → `baaslk://auth?accessToken=…&refreshToken=…&expiresIn=…`; no cookie. Failures/cancel → `/login?error=…` (web) or `baaslk://auth?error=…` (mobile). |
...
| `POST /api/providers/:id/contact` | public | Contact reveal (#64/#655): returns `{ phone, whatsapp, phone2, email }`. The public payloads omit all of these (only `has*` booleans ship) so crawlers can't harvest them; the web reveals them on an explicit "show contact" tap. Rate-limited (`contactReveal`, 20/10 min per IP). Suspended → 404 unless ADMIN. |
