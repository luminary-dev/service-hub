...
| Redis: session-revocation list + rate-limit windows + notification email queue | `redis` (prod `redis_data` volume) | **backed up (#757)**: `scripts/backup-dbs.sh` streams a point-in-time RDB (`redis-cli --rdb -`) into the snapshot as `redis.rdb`, shipped offsite + pruned with the DB dumps. On-host durability comes from AOF (`--appendonly yes --appendfsync everysec`, at-most-1s loss) + RDB on the `redis_data` volume (#571); the snapshot adds an **offsite** copy so a lost volume/host doesn't silently revive revoked sessions (#374) or drop the email queue. Even after a total Redis loss the revocation list re-populates from identity_db's `sessionVersion` on the next bump |
...
- **Restore verification**: every nightly run restores the fresh snapshot into a scratch Postgres container and row-counts each service's main table (`scripts/verify-backup.sh` — fails loudly on a missing/corrupt dump or an empty `User` table; `notification_db`'s `Notification` and `trust_safety_db`'s `Report` counts may legitimately be zero, so only identity's zero-rows case is fatal). It also validates any upload-volume tar in the snapshot is a readable gzip archive (absent tars are fine — R2 mode or none configured), and that a present `redis.rdb` (#757) carries the RDB magic header (absent is fine — a dev run or Redis-less stack). CI's `e2e` job exercises the same backup → restore-verify path on every PR. Still walk the full runbook below as a quarterly drill.
