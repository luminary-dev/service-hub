...
| `IDENTITY_DB_PASSWORD`, `PROVIDER_DB_PASSWORD`, `REVIEW_DB_PASSWORD`, `JOB_DB_PASSWORD`, `REDIS_PASSWORD` | prod compose only (#387) — per-service least-privilege DB role passwords + Redis AUTH. URL-interpolated, so generate URL-safe values (`openssl rand -hex 32`). See [DEPLOYMENT.md](../DEPLOYMENT.md) |
