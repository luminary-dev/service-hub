...
| var | used by |
|---|---|
| `PORT` | every service (defaults: identity 4001, provider 4002, review 4003, job 4004, notification 4005, media 4006, chat 4007, trust-safety 4009, gateway 4000 — 4008 is reserved for search-service) |
| `DATABASE_URL` | identity, provider, review, job, trust-safety |
...
| `IDENTITY_DB_PASSWORD`, `PROVIDER_DB_PASSWORD`, `REVIEW_DB_PASSWORD`, `JOB_DB_PASSWORD`, `TRUST_SAFETY_DB_PASSWORD`, `REDIS_PASSWORD` | prod compose only (#387) — per-service least-privilege DB role passwords + Redis AUTH. URL-interpolated, so generate URL-safe values (`openssl rand -hex 32`). See [DEPLOYMENT.md](../DEPLOYMENT.md) |
